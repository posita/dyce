    ...
    (
        ...
        if [ ! -e "${_VENVSETUP_DIR}/bin/python" ] \
            ...
            if [ "$( "${_VENVSETUP_BASE_PYTHON}" -c 'import sys ; print(sys.version_info.major)' )" -lt 3 ] \
                    || [ "$( "${_VENVSETUP_BASE_PYTHON}" -c 'import sys ; print(sys.version_info.minor)' )" -lt 9 ] ; then
                echo 1>&2 "${0}: Python 3.9+ is required; but $( bash -c "which ${_VENVSETUP_BASE_PYTHON}" ) is:"
